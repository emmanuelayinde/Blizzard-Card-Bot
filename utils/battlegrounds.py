...
website = 'https://hearthstone.blizzard.com/en-gb/battlegrounds'
...
def scrape(webdriver, Service, chrome_options, WebDriverWait, By, EC):
    ...
    for card in all_cards_link:
        ...
        print('pat', path)
        with open(path +"/data/battlegrounds.txt") as f:
            for line in f:
                if line.strip() == card.get_attribute('href'):
                    tweeted = True
                    break
        if tweeted:
            continue  
        else: 
            with open(path +"/data/battlegrounds.txt", 'a') as f:
...
def scrape_card_info(driver, By, url):
    ...
    # //div[@id='MainCardGrid']/div/div[2]/a/div/div/img
